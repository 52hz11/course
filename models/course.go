...
type Course struct {
	...
	ImgPath   string `orm:"column(img_path);size(200);null"`
}
...
func QueryCourse(id int, name string, content string, creator_id int, offset int, limit int) []Course {
	var courses []Course
	o := orm.NewOrm()
	qs := o.QueryTable(new(Course))
	...
	if creator_id != -1 {
		...
		cond = cond.And("CreatorId", creator)
	}
